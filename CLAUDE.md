...
- **Roadmap (aprovado):** dados → serviços reais → triagem → ficha da cliente → tratamentos/pacotes/sessões/evolução → financeiro/relatórios → dashboard → polimento. Etapas 0–4 (dados; site com tratamentos reais; triagem pública + Triagens; ficha da cliente + anamnese) concluídas. Ficha: `src/lib/client-file.ts` (carregador + linha do tempo), `src/lib/client-stage.ts` e `src/lib/anamnesis.ts` (regras puras, em `tests/unit`), `src/components/admin/client/`. Triagem: `src/lib/screening.ts` (regras puras, testadas em `tests/unit`), `src/app/(site)/triagem`, `src/components/screening`, `src/app/admin/(painel)/triagens`. Produção não coleta sem termo de consentimento ATIVO (`/admin/consentimento`). Rota pública de tratamentos: `/tratamentos` (`/servicos` redireciona).
